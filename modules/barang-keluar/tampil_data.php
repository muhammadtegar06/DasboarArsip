<?php
// Tampil Data: Desain Premium & Elegan (Updated View)
...
?>
    ...
    <style>
        ...
        /* 1. Tombol Soft UI (Lebih Elegan) */
        .btn-soft-primary { background-color: #e0e7ff; color: #4338ca; border: none; transition: all 0.2s; }
        .btn-soft-primary:hover { background-color: #c7d2fe; color: #3730a3; transform: translateY(-2px); }
        
        .btn-soft-info { background-color: #cffafe; color: #0891b2; border: none; transition: all 0.2s; }
        .btn-soft-info:hover { background-color: #a5f3fc; color: #0e7490; transform: translateY(-2px); }

        /* 2. Kartu Statistik Modern (Sesuai Gambar Referensi) */
        .card-stat-modern {
            background: white;
        ...
        /* Tema Warna Kartu */
        .theme-cyan .icon-circle-modern { background-color: #ccfbf1; color: #0f766e; } /* Completed Style */
        .theme-green .icon-circle-modern { background-color: #dcfce7; color: #15803d; } /* In-Progress Style */
        .theme-orange .icon-circle-modern { background-color: #ffedd5; color: #c2410c; } /* Upcoming Style */

        /* Garis putus-putus untuk tema Orange */
        .dashed-border {
        ...
        <div class="page-inner py-5">
            <div class="d-flex align-items-left align-items-md-top flex-column flex-md-row">
                ...
                <div class="ml-md-auto py-2 py-md-0">
                    <a href="?module=form_entri_barang_masuk" class="btn btn-white btn-border btn-round shadow-sm font-weight-bold">
    ...
    <div class="page-inner mt--5">
        <div class="card border-0 shadow-sm rounded-lg" style="border-radius: 15px;">
            <div class="card-header bg-white border-0 pt-4 pb-2" style="border-radius: 15px 15px 0 0;">
                <h4 class="card-title font-weight-bold text-dark">Daftar Box & Bantex (Mode Preview)</h4>
            </div>
            <div class="card-body">
                                        ...
                                        <?php if ($data['status'] == 'Diterima') { 
                                            if ($data['sudah_input']) { ?>
                                                <button type="button" 
                                                    ...
                                                    class="btn btn-soft-info btn-round btn-sm font-weight-bold px-4">
                                                    <i class="fas fa-eye mr-2"></i> Lihat
                                                </button>
                                            <?php } else { ?>
                                                <a href="?module=form_input_dokumen&id=<?php echo $data['id_transaksi']; ?>" class="btn btn-soft-primary btn-round btn-sm font-weight-bold px-4">
                                                    <i class="fas fa-file-import mr-2"></i> Input
                                                </a>
                                            <?php } ?>
                                        <?php } else { ?>
                                            <button class="btn btn-light text-muted btn-round btn-sm px-3" disabled><i class="fas fa-lock mr-1"></i> Locked</button>
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php } ?>
                ...
                <div class="modal-body modal-body-elegant">
                    ...
                    <div id="view-divisi" class="view-section active">
                        ...
                        <div class="row justify-content-center">
                            ...
                            <div class="col-md-3 mb-4">
                                <div class="card-stat-modern theme-orange">
                                    <div class="icon-circle-modern">
                                        <div class="dashed-border">
                                            <i class="fas fa-clock"></i>
                                        </div>
                                    </div>
                                    <div class="stat-number-modern">2025</div>
                                    <div class="stat-label-modern">Tahun Arsip</div>
                                </div>
                            </div>
                        </div>
                        
                        <div class="text-center mt-4">
                            <p class="text-muted small"><i class="fas fa-mouse-pointer mr-1"></i> Klik kartu <b>Total Box</b> untuk melihat rincian.</p>
                        </div>
                    </div>

                    <div id="view-box" class="view-section">
                        <div class="d-flex align-items-center mb-4">
                            <button class="btn btn-sm btn-icon btn-light rounded-circle mr-3" onclick="backToDivisi()"><i class="fas fa-arrow-left"></i></button>
                    ...
                    <div id="view-bantex" class="view-section">
                         <div class="d-flex align-items-center mb-4">
                            <button class="btn btn-sm btn-icon btn-light rounded-circle mr-3" onclick="backToBox()"><i class="fas fa-arrow-left"></i></button>
                    ...
                    <div id="view-dokumen" class="view-section">
                        <div class="d-flex align-items-center mb-4">
                            <button class="btn btn-sm btn-icon btn-light rounded-circle mr-3" onclick="backToBantex()"><i class="fas fa-arrow-left"></i></button>
                             <h5 class="font-weight-bold text-dark mb-0" id="lblCurrentBantex">Daftar Dokumen</h5>
                        </div>
                        <div class="card border-0 shadow-sm rounded-lg overflow-hidden">
                            <div class="table-responsive">
                                <table class="table table-hover mb-0">
                                    <thead class="bg-light">
                                        <tr>
                                            <th class="pl-4">Nama Dokumen</th>
                                            <th class="text-center">Tahun</th>
                                            <th class="text-center">File</th>
                                        </tr>
                                    </thead>
                                    <tbody id="containerDokumenList"></tbody>
                                </table>
                            </div>
                        </div>
                    </div>
    ...
    <script>
            ...
            for(let i=1; i<=totalBoxGlobal; i++) {
                ...
                <div class="col-md-3 mb-4">
                    <div class="card item-card h-100 p-3 text-center cursor-pointer transition-hover" onclick="navToBantex(${i})">
                        <div class="item-icon-wrapper bg-icon-box mx-auto mt-2">
                            <i class="fas fa-box fa-lg"></i>
                        </div>
                        <h6 class="font-weight-bold mt-3 mb-0 text-dark">Box ${i}</h6>
        ...
        function navToBantex(boxNum) {
            let html = '';
            // Simulasi jumlah bantex per box
            let bantexPerBox = Math.ceil(totalBantexGlobal / totalBoxGlobal); 
            
            for(let i=1; i<=bantexPerBox; i++) {
                html += `
                <div class="col-md-3 mb-3">
                    <div class="card item-card h-100 p-3 text-center cursor-pointer transition-hover" onclick="navToDokumen(${boxNum}, ${i})">
                        <div class="item-icon-wrapper bg-icon-bantex mx-auto">
                            <i class="fas fa-folder-open fa-lg"></i>
                        </div>
                        <h6 class="font-weight-bold mt-2 mb-0 text-dark">Bantex ${i}</h6>
                    </div>
                </div>`;
            }
            $('#lblCurrentBox').text('Isi Box ' + boxNum);
            $('#containerBantexList').html(html);
            window.currentBoxNum = boxNum; // Simpan state
        ...
        function navToDokumen(boxNum, bantexNum) {
            let html = `
                <tr>
                    <td class="pl-4">
                        <div class="d-flex align-items-center">
                            <div class="btn btn-icon btn-sm btn-light text-danger mr-3"><i class="fas fa-file-pdf"></i></div>
                            <span class="font-weight-bold text-dark">Dokumen_Laporan_${boxNum}_${bantexNum}.pdf</span>
                        </div>
                    </td>
                    <td class="text-center font-weight-bold text-muted">2024</td>
                    <td class="text-center"><a href="#" class="btn btn-sm btn-soft-info rounded-pill px-3"><i class="fas fa-download mr-1"></i> Unduh</a></td>
                </tr>
            `;
